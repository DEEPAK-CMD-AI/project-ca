...
#Functions for open Left-Right fuzzyfication
 ...
 if x<alpha:
      return 1
 if alpha<x and x<=beta: return (beta - x)/(beta alpha)
 else:
   return 0
...
def triangular(x,a,b,c):
 ...
 #Fuzzy Partition
  def partition(x):
       NL = 0; NM = 0 ; NS = 0 ; ZE = 0 PS = 0; PM = 0; PL = 0 NL = openLeft (x, 30,60)
...
# Display the fuzzy values for all fuzzy sets
 out Put= [[NLSD, NMSD, NSSD, ZESD, PSSD, PMSD, PLSD], 
...
PLTC, PMTC, PSTC, NSTC, NLTC = rule(NLSD, NMSD, NSSD, ZESD, PSSD, PMSD, PLSD, NLAC, NMAC,
...
def defuzzyfication (PLTC, PMTC, PSTC, NSTC, NLTC):
    ...
    if PLTC != 0:
         ...
         areaPL, CPL = areaOR (PLTC, 180, 21)

    if PMTC != 0:
         areaPM = area TR (PMTC, 150, 180, 210)
         CPM = 180
    ...
    if NSTC != 0:
        areaNS = area TR (NSTC, 60, 90, 120)
        cNS = 90

    if NLTC !=0:
        areaNL, CNL = areaOL (NLTC, 30, 60)

numerator areaPL cPL + area PM cPM+areaPS cPS + areaNS*CNS + areaNL *cNL 
